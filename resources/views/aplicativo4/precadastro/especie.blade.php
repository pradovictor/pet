...
<div id="form-modal-especie" class="form-modal modal fade" role="dialog" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Especie</h4>
                <button class="close" type="button" data-dismiss="modal">
                    <span>&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="form-especie" method="POST" action="{{config('app.url')}}/action/aplicativo4/precadastro/especie" onsubmit="return false;"
            ...
            <div class="modal-footer">
                ...
                <button id="saveform-especie" type="button" class="btn btn-primary">Salvar</button>
            </div>
        </div>
    </div>
</div>
...
<div class="panel simplecrud-panel">
    <div class="panel-body" id="especie">
    </div>
</div>
@endsection
...
<script type="text/javascript">
    var simpleCRUDObj = SimpleCRUD.load({
        dataTables: {
            ...
            container: "especie",
            formEntity: "form-especie",
            editType: "modal",
            // filterFooter: true,
            addType: @if(Auth::user()->can('PRECADASTRO4-A')) "modal" @else "none" @endif,
            addButtonText: "Adicionar Especie",
            urls: {
                list: "{{config('app.url')}}/action/aplicativo4/precadastro/especie/list",
                delete: "{{config('app.url')}}/action/aplicativo4/precadastro/especie/delete/",
                edit: "{{config('app.url')}}/action/aplicativo4/precadastro/especie/editar/",
                add: "{{config('app.url')}}/action/aplicativo4/precadastro/especie/novo"
            }
        }
    });

    @if(Auth::user()->can('PRECADASTRO4-A') || Auth::user()->can('PRECADASTRO4-E'))
        $('#form-especie').parsley().on('form:success', function () {
            Nitsys.saveForm('form-especie', simpleCRUDObj);
        });
        $('.form-modal').on('hidden.bs.modal', function (e) {
            Nitsys.clearForm('form-especie');
            $('#form-especie').parsley().reset();
