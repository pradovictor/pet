...
<div id="form-modal-pelagem" class="form-modal modal fade" role="dialog" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Pelagem</h4>
                <button class="close" type="button" data-dismiss="modal">
                    <span>&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="form-pelagem" method="POST" action="{{config('app.url')}}/action/aplicativo4/precadastro/pelagem" onsubmit="return false;"
            ...
            <div class="modal-footer">
                ...
                <button id="saveform-pelagem" type="button" class="btn btn-primary">Salvar</button>
            </div>
        </div>
    </div>
</div>
...
<div class="panel simplecrud-panel">
    <div class="panel-body" id="pelagem">
    </div>
</div>
@endsection
...
<script type="text/javascript">
    var simpleCRUDObj = SimpleCRUD.load({
        dataTables: {
            ...
            container: "pelagem",
            formEntity: "form-pelagem",
            editType: "modal",
            // filterFooter: true,
            addType: @if(Auth::user()->can('PRECADASTRO4-A')) "modal" @else "none" @endif,
            addButtonText: "Adicionar Pelagem",
            urls: {
                list: "{{config('app.url')}}/action/aplicativo4/precadastro/pelagem/list",
                delete: "{{config('app.url')}}/action/aplicativo4/precadastro/pelagem/delete/",
                edit: "{{config('app.url')}}/action/aplicativo4/precadastro/pelagem/editar/",
                add: "{{config('app.url')}}/action/aplicativo4/precadastro/pelagem/novo"
            }
        }
    });

    @if(Auth::user()->can('PRECADASTRO4-A') || Auth::user()->can('PRECADASTRO4-E'))
        $('#form-pelagem').parsley().on('form:success', function () {
            Nitsys.saveForm('form-pelagem', simpleCRUDObj);
        });
        $('.form-modal').on('hidden.bs.modal', function (e) {
            Nitsys.clearForm('form-pelagem');
            $('#form-pelagem').parsley().reset();
