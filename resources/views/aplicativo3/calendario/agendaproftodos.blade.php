...
@php($profissionais = DB::table('a3profissional')->select('*')->get())
@foreach($profissionais as $profissional)
<div class="row-md-5">
    {{-- <div class="tl-label bs-label {{$profissional->cor}}">{{$profissional->nome}}</div> --}}
    <div class="form-control" style="color: white; font-weight: bold; text-align: center; background: {{$profissional->cor}};">{{$profissional->nome}}</div>
</div>
@endforeach
</div>
...
<script type="text/javascript">
    ...
    @php($registros = DB::table('a3agenda')->select('*')->get())
            ...
            events: [
                <?php
                    foreach($registros as $registro){
                        ...
                        // pegando cor - banco profissional
                        $cores = DB::table('a3profissional')->select('*')->where('id', '=', $registro->a3profissional_id)->get();
                        foreach($cores as $cor){ $xcor = $cor->cor;}
                        // fim da cor
